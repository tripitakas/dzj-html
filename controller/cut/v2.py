        ...
        if len(a_c['Down']) == 0:
            ...
            if len(b_c['Up']) >= 2:
                ...
                for i in range(0, len(idx_sorted)):
                    ...
                    mark_sub_col_id(char_list, connection, subidx, i + 1, -1)
                    if subidx != idx:
...
def mark_sub_col_id(char_list, connection, idx, marker, direction):
    """标记夹注子列的序号"""
    A = char_list[idx]
    a_c = connection[idx]
    if A['sub_col_id'] != 0:
    ...
    if direction == 1:
        if len(a_c['Down']) == 0:
            ...
            b_c = connection[next_idx]
            if len(b_c['Up']) == 1:
                mark_sub_col_id(char_list, connection, next_idx, marker, 1)
            elif len(b_c['Up']) >= 2:
                return
        else:
            return
...
def calc_order(char_list, indices):
    """根据列号、子列号等，计算列内序号"""
    candidates = [{'ch_id': 0, 'sub_col_id': 1, 'note_id': 1},
                  {'ch_id': 1, 'sub_col_id': 0, 'note_id': 0}]
    for order in range(0, len(indices)):
        flag = 0
        for cnddt in candidates:
            for i in indices:
                if char_list[i]['ch_id'] == cnddt['ch_id'] and \
                        char_list[i]['sub_col_id'] == cnddt['sub_col_id'] and \
                        char_list[i]['note_id'] == cnddt['note_id']:
                    flag = 1
                    char_list[i]['column_order'] = order + 1
                    if cnddt['sub_col_id'] != 0:
                        # 这个字是小字
                        candidates = [{'ch_id': char_list[i]['ch_id'], 'sub_col_id': char_list[i]['sub_col_id'],
                                       'note_id': char_list[i]['note_id'] + 1},
                                      {'ch_id': char_list[i]['ch_id'], 'sub_col_id': char_list[i]['sub_col_id'] + 1,
                                       'note_id': 1},
                                      {'ch_id': char_list[i]['ch_id'] + 1, 'sub_col_id': 0, 'note_id': 0},
                                      {'ch_id': char_list[i]['ch_id'] + 1, 'sub_col_id': 1, 'note_id': 1}]
                    else:
                        candidates = [{'ch_id': char_list[i]['ch_id'], 'sub_col_id': 1, 'note_id': 1},
                                      {'ch_id': char_list[i]['ch_id'] + 1, 'sub_col_id': 0, 'note_id': 0},
                                      {'ch_id': char_list[i]['ch_id'] + 1, 'sub_col_id': 1, 'note_id': 1}]
                    break
            if flag:
...
def show(char_list, coordinate_char_list, indices, ax, filename):
    ...
    for i in indices:
        ...
        #        plt.text(xleft,-ydown,str(A['column_id'])+'-'+str(A['ch_id'])+'-'+str(A['sub_col_id'])+'-'+str(A['note_id']))
...
def has_note_in_column(chars_in):
    return [c for c in chars_in if c['sub_col_id']]
