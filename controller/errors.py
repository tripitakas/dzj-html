#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
@desc 定义后端API的错误码和数据库常用函数
@time: 2018/10/23
"""

need_login = 403, '尚未登录'

db_error = 10000, '数据库访问出错'
mongo_error = 20000, '文档库访问出错'

not_allowed_empty = 1000, '不允许为空'
not_allowed_both_empty = 1001, '%s和%s不允许同时为空'
invalid_name = 1002, '姓名应为2~5个汉字，或3~20个英文字母（可含空格和-）'
invalid_phone = 1003, '手机号码格式有误'
invalid_email = 1004, '邮箱格式有误'
invalid_password = 1005, '密码应为6至18位由数字、字母和英文符号组成的字符串，不可以为纯数字或纯字母'
invalid_range = 1006, '数据范围应为[%s, %s]'
need_phone_or_email = 1007, '没有指定手机或邮箱'
need_password = 1008, '没有指定密码'

no_user = 1010, '没有此账号'
user_exists = 1011, '账号已存在'
incorrect_password = 1012, '密码错误'
unauthorized = 1013, '没有权限'
auth_changed = 1014, '授权信息已改变，请您重新登录'
no_change = 1015, '没有发生改变'
incomplete = 1016, '信息不全'
invalid_parameter = 1017, '无效的参数'
no_object = 1018, '对象不存在或已删除'

task_locked = 2000, '本任务已被领走，请领取新的任务'
task_uncompleted = 2001, '您还有未完成的任务，请继续完成后再领取新的任务'
task_changed = 2002, '本任务的状态已改变'
