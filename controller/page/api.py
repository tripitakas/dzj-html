...
class TextReviewApi(PageHandler):
    ...
    def post(self, task_id):
        ...
        try:
            ...
            # 更新page
            txt_html = data.get('txt_html', '').strip('\n')
            update = {'text': self.html2txt(txt_html), 'txt_html': txt_html}
            self.db.page.update_one({'name': self.page_name}, {'$set': update})
            self.send_data_response()
...
class TextHardApi(PageHandler):
    ...
    def post(self, task_id):
        ...
        try:
            ...
            # 更新page
            txt_html = data.get('txt_html', '').strip('\n')
            self.db.page.update_one({'name': self.task['doc_id']}, {'$set': {
                'text': self.html2txt(txt_html), 'txt_html': txt_html
            }})
            # 提交任务
            if data.get('submit'):
                if self.mode == 'do':
...
class TextEditApi(PageHandler):
    ...
    def post(self, page_name):
        ...
        try:
            ...
            self.add_op_log('save_edit_text', context=page_name, target_id=self.page['_id'])
            if data.get('submit'):
