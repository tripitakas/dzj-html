...
class TestTextTaskSegment(APITestCase):
    ...
    def test_gen_segments_simple(self):
        ...
        s = Proof.gen_segments('N \U0002e34f', [])  # GL_1056_5_6
        ...
        s = Proof.gen_segments('平等無傾囬邪自致𥼶氏云', [])  # GL_1260_4_8
        self.assertEqual([c.get('type') for c in s], ['same', 'variant', 'same'])
        self.assertEqual(s[1].get('ocr'), ['%F0%A5%BC%B6'])

    def test_gen_segments_simple_chars(self):
        """测试文字校对生成标记文本--基本字框测试"""
        chars = [
        ...
        s = Proof.gen_segments('致𥼶氏 云', chars)
        self.assertEqual([c.get('txt') for c in chars], ['致', '𥼶', '氏', '云'])
        self.assertEqual([c.get('ocr') for c in s], [['%E8%87%B4'], ['%F0%A5%BC%B6'], ['%E6%B0%8F', '+', '%E4%BA%91']])
    ...
    def test_gen_segments_mismatch_lines(self):
        ...
        self.assertEqual(params['mismatch_lines'], ['b1c6', 'b1c7'])
