...
class TestDataLock(APITestCase):
    ...
    def setUp(self):
        ...
        self.add_users_by_admin(
            [dict(email=r[0], name=r[2], password=r[1]) for r in [u.expert2]],
            '切分审定员,单元测试用户'
        )
        self.add_users_by_admin(
    ...
    def test_data_lock_special(self):
        ...
        # 测试不能发布切分审定任务
        self.login_as_admin()
        r = self.publish_tasks(dict(task_type='cut_review', doc_ids=ready_ids, pre_tasks=[]))
        ...
        self.assertEqual(e.lock_level_unqualified[0], self.parse_response(r).get('err')[0], msg=task_type)
